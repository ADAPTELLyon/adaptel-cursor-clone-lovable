...
import { AjoutSuiviClientDialog } from "./AjoutSuiviClientDialog"
...
export function CandidatSuiviTab({ candidatId, secteurs }: Props) {
  ...
  const renderCarte = (item: InterdictionPriorite) => {
    const nom = (item as any)?.client?.nom?.toLowerCase() || ""
    ...
    return (
      <li key={item.id} className="border rounded p-3 text-sm bg-white shadow-sm space-y-2 relative">
        <div className="flex justify-between items-start">
          <div>
            <div className="font-semibold text-base">{(item as any)?.client?.nom}</div>
